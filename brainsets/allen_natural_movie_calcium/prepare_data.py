...
from brainsets_utils.taxonomy.mice import *
from brainsets_utils.taxonomy.subject import *
...
def extract_nm1_stim_trials(nwbfile):
    ...
    trials = Interval(
        ...
        frame_number=np.array(frame_number).astype(np.int64),
...
def main():
    ...
    db = DatasetBuilder(
        ...
        experiment_name="allen_brain_observatory_calcium_NM1",
        origin_version="v2",
        ...
        "Allen Institute Brain Observatory with stimulus natural movie 1."
        "Only in VISp and excitatory crelines",
    )
    ...
    for count, file_path in enumerate(find_files_by_extension(nwb_file_path, ".nwb")):
        ...
        with db.new_session() as session:
            ...
            session.register_split("train", nm1_train_trials)
            session.register_split("valid", nm1_valid_trials)
            session.register_split("test", nm1_test_trials)
