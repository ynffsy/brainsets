...
def get_stim_data(meta_table, timestamps):
    ...
    stimuli_segments.drifting_class = np.round(trials.orientation / 45.0).astype(np.int64)
    ...
    for count, curr_sess_id in enumerate(sess_ids):
        ...
        with db.new_session() as session:
            ...
            traces = IrregularTimeSeries(
                ...
                unit_index = np.tile(np.arange(0,curr_num_rois).astype(np.int16), (curr_num_frames,)),
            ...
            trials, stimuli_events, stimuli_segments = get_stim_data(master_stim_table, roi_ts)
