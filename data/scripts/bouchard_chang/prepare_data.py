    ...
    for i in range(256):
        ...
        channels.append(
                Channel(
                    ...
                    local_index=i,
                    relative_x_um=electrodes_dynaTable.x.data[i], # x, y, z coordinates are not always present in the dataset, e.g. missing for EC9 & GP33
...
def extract_trials(trials: TimeIntervals, epochs: TimeIntervals, invalid_times: TimeIntervals):
    ...
    invalid_trial = torch.tensor([False]*trials.start_time.data.shape[0], dtype=torch.bool)
    invalid_epoch = torch.tensor([False]*epochs.start_time.data.shape[0], dtype=torch.bool)
    ...
    if all((trial_start_time[1:] - trial_stop_time[:-1]) >= 0):
        ...
        logging.info(f"there is/are {np.sum((trial_start_time[1:] - trial_stop_time[:-1]) < 0)} trial overlapped")
    ...
    for i in range(epoch_start_time.shape[0]):
        if not (invalid_times == None):
            if any(np.logical_and(invalid_start_time <= epoch_stop_time[i], invalid_stop_time >= epoch_start_time[i])):
...
if __name__ == "__main__":
    ...
    for file_path in tqdm(sorted(find_files_by_extension(raw_folder_path, extension))):
        ...
        with NWBHDF5IO(file_path, 'r', load_namespaces=True) as io:
            ...
            cv_trials, rest_period, session_start, session_end = extract_trials(trials, epochs, invalid_times)
            ...
            valid_slices, valid_slice_labels = collect_slices_wChecks(data, valid_segments)
            ...
            char_counts = {}
            _, char_counts['train'] = np.unique(np.array(train_slice_labels), return_counts=True)
            _, char_counts['valid'] = np.unique(np.array(valid_slice_labels), return_counts=True)
            _, char_counts['test'] = np.unique(np.array(test_slice_labels), return_counts=True)
            ...
            for buckets, fold in [(train_slices, 'train'), (valid_slices, 'valid'), (test_slices, 'test')]:
                for i, sample in enumerate(buckets):
                    ...
                    trial_descriptions.append(TrialDescription(
                        id=basename,
                        footprints={fold: os.path.getsize(path),},
