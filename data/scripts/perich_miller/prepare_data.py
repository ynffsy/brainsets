...
def extract_spikes(mat_dict: dict, prefix: str):
    ...
    for i in range(len(units)):
        ...
        unit_name = f"{prefix}/{array}_{channel_label}/sorted_{sorted_id:02}"

        all_names = [x["unit_name"] for x in unit_meta]
...
def extract_behavior(mat_dict):
    ...
    behavior = IrregularTimeSeries(
        ...
        / 20.0,  # todo: improve this so it does not have to be hard coded.
...
def extract_trial_metadata_co(mat_dict: dict, behavior: IrregularTimeSeries):
    ...
    default_value = values["endTime"][:, 0] + 1.0
    ...
    for i in range(len(trials)):
        ...
        if check_co_baseline_trial_validity(trials[i]):
            behavior_type[
                (behavior.timestamps >= trials.target_on_time[i])
                & (behavior.timestamps < trials.go_cue_time[i])
            ] = REACHING.CENTER_OUT_HOLD
            behavior_type[
                (behavior.timestamps >= trials.go_cue_time[i])
                & (behavior.timestamps < trials.target_acq_time[i])
            ...
            stimuli_reach_direction_deg.extend(
                [trials.target_dir[i], np.mod(trials.target_dir[i], 360.0)]
            )
        else:
        ...
        stimuli_timestamps.extend(
            [
                ...
                trials.go_cue_time[i],
                trials.target_acq_time[i],
            ]
        )
        stimuli_type.extend(
            [REACHING.TARGET_ON, REACHING.GO_CUE, REACHING.TARGET_ACQUIRED]
        )
...
def extract_trial_metadata_rt(mat_dict: dict, behavior: IrregularTimeSeries):
    ...
    for i in range(len(trials)):
        ...
        behavior_type[
            (behavior.timestamps >= trials.start[i])
            & (behavior.timestamps < trials.go_cue_time_1[i])
        ] = REACHING.HOLD
        behavior_type[
            (behavior.timestamps >= trials.go_cue_time_1[i])
            & (behavior.timestamps < trials.end[i])
        ...
        stimuli_timestamps.extend(
            [
                trials.go_cue_time_1[i],
                trials.go_cue_time_2[i],
                trials.go_cue_time_3[i],
                trials.go_cue_time_4[i],
            ]
        )
        stimuli_type.extend(
            [
...
def filter_buckets(buckets):
    ...
    for bucket in buckets:
        ...
        outlier_ratio = torch.sum(
            bucket.behavior.type == REACHING.OUTLIER
        ).item() / len(bucket.behavior)
...
def check_co_baseline_trial_validity(trial, min_duration=0.5, max_duration=6.0):
    ...
    cond3 = (trial["end"] - trial["target_on_time"]) > min_duration and (
        trial["end"] - trial["target_on_time"]
    ) < max_duration
...
def check_rt_baseline_trial_validity(trial, min_duration=2.0, max_duration=10.0):
    ...
    # check if the duration of the trial is between min_duration and max_duration
    cond3 = (trial["end"] - trial["start"]) > min_duration and (
    ...
    for file_path in tqdm(sorted(find_files_by_extension(raw_folder_path, extension))):
        ...
        if task == "CO":
            (
                ...
                stimuli_events,
                stimuli_segments,
            ) = extract_trial_metadata_co(mat_dict, behavior)
        elif task == "RT":
            (
                trials,
                behavior,
                stimuli_events,
                stimuli_segments,
            ) = extract_trial_metadata_rt(mat_dict, behavior)
        else:
        ...
        if "CO" in file_path:
            successful_trials = list(
                filter(check_co_baseline_trial_validity, data.trials)
            )
        elif "RT" in file_path:
            successful_trials = list(
                filter(check_rt_baseline_trial_validity, data.trials)
            )

        logging.info(
            f"Found {len(successful_trials)} out of {len(data.trials)} trials that were successful."
        )
        (
            train_trials,
            valid_trials,
